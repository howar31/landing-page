    ...
    >
      ...
      <div>
        ...
        {!loading && !error && data && data.length > 0 &&
          data.slice(0, 3).map((post, index, posts) => (
            <a
              key={post.link}
              href={post.link}
              target="_blank"
              rel="noopener noreferrer"
              className={`grid gap-x-[14px] feed:gap-x-[18px] gap-y-2 py-4 text-white/85 no-underline transition-colors duration-200 hover:text-white group grid-cols-[80px_1fr] feed:grid-cols-[110px_1fr]${
