...
/**
 * Read a cache entry ignoring TTL. Used as a fallback when a fresh fetch
 * fails (e.g., GitHub API rate-limit) so the UI can still show last-known
 * data rather than disappearing entirely. Caller must treat the result as
 * possibly stale.
 */
